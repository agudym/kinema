...
if __name__ == "__main__":
    ...
    def function_jax(x, ftype):
        # sqrt(1/N) is necessary to make termination constant work fine
        if ftype == 0:
            residuals = (x - x_gt) / N**0.5
        else:
            residuals = (((x - x_gt)**2 + (x[0] - x_gt[0])**2) / N)**0.5
        if ftype >= 2:
            # Add outlier residual
    ...
    for jacobian_struct in ("DENSE", "SPARSE"):
        if jacobian_struct == "DENSE":
            ...
            # Very inefficient sparse-jacobian building for simple test,
            # the right way is to build sparse jacobian directly, check
            # https://github.com/mfschubert/sparsejac for example
            jacobian = lambda x, *args: scipy.sparse.coo_array(jacobian_jax(x, *args))
        print(f"Jacobian TYPE: {jacobian_struct}")
        ...
        for ftype, losstype in zip((0, 1, 2, 3), (RobustLossType.Linear, RobustLossType.Linear, RobustLossType.Cauchy, RobustLossType.Linear)):
            ...
            # ftype 3: outlier with zero weight (should be ignored)
            weights = None
            if ftype == 3:
                weights = np.ones(N + 1)
                weights[-1] = 0  # zero weight for outlier residual
            ...
            print(f"\t\tMax(x - x_gt): {x_err[x_err_max_id]} in element #{x_err_max_id}")
